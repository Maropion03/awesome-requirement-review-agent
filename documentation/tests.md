...
curl -fsS https://<deployment>/api/formats
```

A model-backed production review requires a user-owned Key and is not part of unauthenticated CI.

`.github/workflows/ci.yml` runs backend, frontend, build, and npm audit checks on pushes and pull requests. A manual dispatch can receive `smoke_url` and verify the public SPA, health response, and three-format catalog from an external GitHub-hosted runner.
