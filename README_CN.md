...
- Product Context Pack：在浏览器保存产品概览、业务目标、目标用户、成功指标和历史决策，启用后注入六维评审。
...
接口格式、Base URL、正文模型、视觉模型、预设、Key 和 Product Context 会以明文写入当前浏览器的 `localStorage`，刷新页面或重启浏览器后仍然保留。Product Context 可针对本次评审停用而不删除；启用时，它与 PRD 一起经过 Vercel Function 转发到配置的模型端点。项目不会把这些内容写入 Cookie、服务端数据库、文件、分析工具或日志。在公共或共享设备上应使用对应的清除操作，或清除该站点的浏览器数据。
...
模型目录会变化，因此前端允许用户分别填写正文评审模型和 PDF 流程图视觉模型。视觉模型留空时复用正文模型；智谱官方端点上的 GLM-5.3 旧配置会使用 `glm-4.6v-flash` 识别流程图，因为 GLM-5.3 只支持文本输入。配置页会常驻提示已知的纯文本模型，启动 PDF 评审前还会弹窗确认正文与视觉模型的分工。用户可以显式填写其他视觉模型覆盖默认值。
...
上传上限为 3.5MB。PDF 正文在浏览器中提取，最多选择 4 个图片密集页面压缩为流程图候选，因此原始 PDF 字节不会经过 Vercel Firewall。独立视觉模型只调用一次，先输出受校验的节点和连线，再由代码确定性生成 Mermaid，供正文模型驱动的六个 Reviewer 复用；视觉调用失败时降级为纯文本评审。Product Context 最多 2 万字符，分为五类固定来源；报告中的引用会校验并标注来自当前 PRD 或具体 Context 字段。正文上限为 8 万字符，扫描件仍需先完成 OCR。
